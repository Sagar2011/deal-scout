...
const analysis: StartupAnalysis = {
  ...
  criteria: { workflowClarity: 0.9, smbFit: 0.9, technicalDepth: 0.4, signalStrength: 0.6, whyNow: 0.8 },
};
...
test("scores and recommends a sufficiently evidenced thesis match", () => {
  ...
  assert.equal(recommend(score, [{ claim: "YC listing", url: candidate.sourceUrl, source: "Y Combinator", capturedAt: "2026-09-02T00:00:00.000Z" }]).decision, "Watch");
});
...
test("accepts a structured LLM analysis", async () => {
  const analyzer = new OpenAiAnalyzer("test-key", {
    ...
    async post() {
      return { data: { choices: [{ message: { content: JSON.stringify(analysis) } }] } };
    },
  });
...
test("renders cited Markdown with a clear call", () => {
  const memo = renderMemo({ candidate, evidence: [{ claim: candidate.description, url: candidate.sourceUrl, source: "Y Combinator", capturedAt: "2026-09-02T00:00:00.000Z" }], analysis, score: scoreAnalysis(analysis), recommendation: { decision: "Watch", rationale: "Promising workflow fit.", mindChanges: ["Verify retention.", "Verify team depth."] } });
...
test("writes evidence, analysis, and a memo for one candidate", async () => {
  ...
  try {
    const summary = await runPipeline({ topic: "AI agents for SMBs", rootDir, candidates: [candidate] });
    assert.equal(summary.completed, 1);
    const memo = await readFile(join(summary.runPath, "memos", "acme-agent.md"), "utf8");
    assert.match(memo, /# Acme Agent/);
    assert.ok(await readFile(join(summary.runPath, "evidence", "acme-agent.json"), "utf8"));
    assert.ok(await readFile(join(summary.runPath, "analysis", "acme-agent.json"), "utf8"));
  } finally {
