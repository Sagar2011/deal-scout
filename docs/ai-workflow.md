...
The fallback analysis is deterministic so a reviewer can run the project without an API key. With OpenRouter enabled, one small pre-sourcing call creates two to four generic, related search queries and saves them in `query-plan.json`; no sector-specific alias table is maintained. The LLM then structures the qualitative research at `temperature: 0`; final scores are independently calibrated from captured evidence, candidate metadata, and verified profile facts. This prevents a different free-model response from materially changing a score when the saved evidence is unchanged.
